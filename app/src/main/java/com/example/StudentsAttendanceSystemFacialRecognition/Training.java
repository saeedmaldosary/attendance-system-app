...
public class Training extends AppCompatActivity {
    ...
    private FaceDetectionProcessor fDetectionProcessor;
    private CameraSource cameraSource = null;
    private CameraSourcePreview preview;
    private GraphicOverlay gOverlay;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gOverlay = findViewById(R.id.overlay);
    ...
    private void capture() {
        List<FirebaseVisionFace> faces = fDetectionProcessor.getDetectedFaces();
        Bitmap original = fDetectionProcessor.getOriginalCameraImage();
        int numFaces = faces.size();
        if(numFaces != 1) {
            Toast.makeText(Training.this, numFaces + " faces detected!", Toast.LENGTH_LONG).show();
        }
        else {
            Bitmap faceBmp = fDetectionProcessor.getFaceBitmap();
            if(faceBmp != null){
                personRecogniser.savePic(faceBmp, nameP);
    ...
    private void createCameraSource() {
        if (cameraSource == null) {
            cameraSource = new CameraSource(this, gOverlay);
        }

        try {
            fDetectionProcessor = new FaceDetectionProcessor();
            cameraSource.setMachineLearningFrameProcessor(fDetectionProcessor);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void startCameraSource() {
        if (cameraSource != null) {
            try {
                ...
                preview.start(cameraSource, gOverlay);
            } catch (IOException e) {
